...
#define SSID "Scott's iPhone"                  //type your own SSID name
#define PASSWORD "STC123stc"        //type your own WIFI password


#define Temp_maxlimit  20        // type your Maximum temperature 
#define Temp_minlimit  18         // type your Minimum temperature 
...
void loop() 
{
  ...
  Control ();               //Control motor according to the result of the previous function
    ...
    while ( Serial.available() > 0 ) {
      char s0rx = Serial.read();    
      _cell.print(s0rx);
    }
    delay(10);
    while ( _cell.available() > 0) {
      char s1rx = _cell.read();
      Serial.print(s1rx);
    }
